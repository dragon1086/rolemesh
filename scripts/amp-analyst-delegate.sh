#!/usr/bin/env bash
# amp-analyst-delegate.sh — amp-analyst 위임 실행기 (auto-generated by RoleMesh)
#
# 사용법:
#   scripts/amp-analyst-delegate.sh [옵션...] -p "작업 내용"
#
# BatchCooldown / CircuitBreaker / Throttle 자동 적용.

set -euo pipefail

SOURCE="${BASH_SOURCE[0]}"
while [[ -L "$SOURCE" ]]; do
    SCRIPT_DIR="$(cd -P "$(dirname "$SOURCE")" && pwd)"
    SOURCE="$(readlink "$SOURCE")"
    [[ "$SOURCE" != /* ]] && SOURCE="$SCRIPT_DIR/$SOURCE"
done
SCRIPT_DIR="$(cd -P "$(dirname "$SOURCE")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
PROVIDER="anthropic"
AGENT_NAME="amp-analyst"

# Python 경로 자동 탐색 (venv 우선)
if [[ -f "$REPO_ROOT/.venv/bin/python3" ]]; then
    PYTHON="$REPO_ROOT/.venv/bin/python3"
elif [[ -f "$REPO_ROOT/venv/bin/python3" ]]; then
    PYTHON="$REPO_ROOT/venv/bin/python3"
else
    PYTHON="python3"
fi

if [[ $# -eq 0 ]]; then
    echo "사용법: $0 [옵션...] -p <prompt>" >&2
    echo "  예시: $0 -p '작업 내용'" >&2
    echo "" >&2
    echo "이 스크립트는 RoleMesh가 $AGENT_NAME 에이전트에 위임할 때 사용하는 실행기입니다." >&2
    exit 1
fi

TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
echo "[$AGENT_NAME-delegate] 📋 위임 시작: $TIMESTAMP" >&2
echo "[$AGENT_NAME-delegate] 인수: $*" >&2

# ── 1. Batch Cooldown 체크 ────────────────────────────────────────────────────
COOLDOWN_RESULT=$("$PYTHON" -c "
import sys
sys.path.insert(0, '$REPO_ROOT/src')
try:
    from rolemesh.adapters.batch_cooldown import BatchCooldown
    bc = BatchCooldown()
    remaining = bc.acquire()
    if remaining > 0.0:
        print(f'WAIT:{remaining:.1f}')
    else:
        print('OK:0')
except Exception as e:
    print(f'ERR:{e}')
")

CD_STATUS="${COOLDOWN_RESULT%%:*}"
CD_DETAIL="${COOLDOWN_RESULT#*:}"

if [[ "$CD_STATUS" == "WAIT" ]]; then
    WAIT_INT=$(printf "%.0f" "$CD_DETAIL")
    echo "[$AGENT_NAME-delegate] ⏳ 배치 쿨다운: ${CD_DETAIL}s 대기 중..." >&2
    sleep "$WAIT_INT"
fi

if [[ "$CD_STATUS" == "ERR" ]]; then
    echo "[$AGENT_NAME-delegate] ⚠️  BatchCooldown 체크 실패 (${CD_DETAIL}) — 체크 없이 진행" >&2
fi

# ── 2. Circuit Breaker 체크 ───────────────────────────────────────────────────
CB_RESULT=$("$PYTHON" -c "
import sys
sys.path.insert(0, '$REPO_ROOT/src')
try:
    from rolemesh.adapters.circuit_breaker import ProviderCircuitBreaker, CBState
    cb = ProviderCircuitBreaker()
    state = cb.get_state('$PROVIDER')
    remaining = cb.cooldown_remaining('$PROVIDER')
    if state == CBState.OPEN:
        print(f'OPEN:{remaining}')
    else:
        print(f'OK:{state.value}')
except Exception as e:
    print(f'ERR:{e}')
")

CB_STATUS="${CB_RESULT%%:*}"
CB_DETAIL="${CB_RESULT#*:}"

if [[ "$CB_STATUS" == "OPEN" ]]; then
    echo "[$AGENT_NAME-delegate] ❌ Circuit Breaker OPEN — $PROVIDER 차단 중 (${CB_DETAIL}s 남음)" >&2
    exit 1
fi

if [[ "$CB_STATUS" == "ERR" ]]; then
    echo "[$AGENT_NAME-delegate] ⚠️  CB 체크 실패 (${CB_DETAIL}) — 체크 없이 진행" >&2
fi

# ── 3. Throttle 체크 ─────────────────────────────────────────────────────────
THROTTLE_RESULT=$("$PYTHON" -c "
import sys
sys.path.insert(0, '$REPO_ROOT/src')
try:
    from rolemesh.adapters.throttle import TokenBucketThrottle
    t = TokenBucketThrottle()
    result = t.acquire('$PROVIDER')
    if result is True:
        print('OK:0')
    else:
        print(f'WAIT:{result:.1f}')
except Exception as e:
    print(f'ERR:{e}')
")

TH_STATUS="${THROTTLE_RESULT%%:*}"
TH_DETAIL="${THROTTLE_RESULT#*:}"

if [[ "$TH_STATUS" == "WAIT" ]]; then
    WAIT_INT=$(printf "%.0f" "$TH_DETAIL")
    echo "[$AGENT_NAME-delegate] ⏳ Throttle: ${TH_DETAIL}s 대기 중..." >&2
    sleep "$WAIT_INT"
fi

if [[ "$TH_STATUS" == "ERR" ]]; then
    echo "[$AGENT_NAME-delegate] ⚠️  Throttle 체크 실패 (${TH_DETAIL}) — 체크 없이 진행" >&2
fi

# ── 4. 명령 실행 ──────────────────────────────────────────────────────────────
echo "[$AGENT_NAME-delegate] ✅ CB=${CB_DETAIL}, Throttle=${TH_STATUS} — python3 ~/amp/amp/interfaces/cli.py 실행" >&2
DELEGATE_EXIT=0
python3 ~/amp/amp/interfaces/cli.py "$@" || DELEGATE_EXIT=$?

# ── 5. 배치 완료 시간 기록 ───────────────────────────────────────────────────
"$PYTHON" -c "
import sys
sys.path.insert(0, '$REPO_ROOT/src')
try:
    from rolemesh.adapters.batch_cooldown import BatchCooldown
    BatchCooldown().record_complete()
except Exception:
    pass
" 2>/dev/null || true

exit "$DELEGATE_EXIT"
