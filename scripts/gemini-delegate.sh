#!/usr/bin/env bash
# gemini-delegate.sh — gemini 위임 실행기 (auto-generated by RoleMesh)
#
# 사용법:
#   scripts/gemini-delegate.sh [옵션...] -p "작업 내용"
#
...
# ── 4. 명령 실행 ──────────────────────────────────────────────────────────────
echo "[$AGENT_NAME-delegate] ✅ CB=${CB_DETAIL}, Throttle=${TH_STATUS} — gemini -p 실행" >&2
DELEGATE_EXIT=0
gemini -p "$@" || DELEGATE_EXIT=$?
